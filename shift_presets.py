...
from shift_constants import DEFAULT_BASE_DEMAND, DEFAULT_COST_PER_SHIFT, DEFAULT_PEAK_HEIGHT, DEFAULT_SHIFT_LENGTH, GAP_EXAMPLE
...
SETTING_SPECS = {
    "shift_length_slider": SettingSpec("len", int, DEFAULT_SHIFT_LENGTH, 3, 16),
    ...
    "cost_slider": SettingSpec("cost", float, DEFAULT_COST_PER_SHIFT, 50.0, 1000.0),
...
def apply_preset(shift_length, n_peaks, peak_conc, seed, base_demand, peak_height, wrap):
    st.session_state["shift_length_slider"] = shift_length
...
def apply_gap_example():
    apply_preset(
        GAP_EXAMPLE["shift_length"], GAP_EXAMPLE["n_peaks"], GAP_EXAMPLE["peak_conc"],
...
def sync_query_params(shift_length, n_peaks, peak_conc, seed, base_demand, peak_height, cost, wrap):
    try:
        st.query_params["len"] = str(shift_length)
